...
class Grid:
    def __init__(self, rows, columns, size, main,screen):
        self.GAME = main
        self.screen=screen
        self.y = rows
        self.x = columns
    ...
    def drawGrid(self):
        ...
        gridBg.fill((0, 0, 0))  # Fill the background with black

        # Draw a plain green grid
        for i in range(self.y):
            for j in range(self.x):
                color = (0, 128, 0)  # Green color
                rect = pygame.Rect(80 + j * 80, 80 + i * 80, 80, 80)
                pygame.draw.rect(gridBg, color, rect)
                pygame.draw.rect(gridBg, (0, 0, 0), rect, 1)  # Add black grid lines

        return gridBg


        # Other drawing code for tokens, scores, and end screen
